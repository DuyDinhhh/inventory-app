...
const CustomerEdit = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setImagePreview(data.photo || "/assets/img/customers/default.webp");
        setImageFile(null);
      } catch (err) {
        setCustomer(null);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await CustomerService.update(id, submitData);
      if (
        ...
        toast.success("Customer updated successfully!", {
          autoClose: 700,
          onClose: () => navigate(`/customer/show/${id}`),
        });
      } else {
        setErrors({
          general: "Customer update failed.",
        });
      }
    } catch (err) {
      setErrors(
        err.response?.data?.errors || { general: "Failed to update customer." }
      );
    }
    setSaving(false);
  };
