...
const CustomerCreate = () => {
  ...
  const [imagePreview, setImagePreview] = useState(
    "/assets/img/customers/default.webp"
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await CustomerService.store(submitData);
      if (
        ...
        setErrors({
          general: "Customer creation failed.",
        });
      }
    } catch (err) {
      setErrors(
        err.response?.data?.errors || { general: "Failed to create customer." }
      );
    }
    setSaving(false);
  };
