...
const PurchaseUpdate = () => {
  ...
  // Load suppliers, products, and purchase on mount
  useEffect(() => {
    setLoading(true);
    Promise.all([
      SupplierService.index(),
      ProductService.index(),
      PurchaseService.show(id),
    ])
      .then(([suppRes, prodRes, purchaseRes]) => {
        setSuppliers(suppRes.data || suppRes);
        setProducts(prodRes.data || prodRes);
        setFilteredProducts(prodRes.data || prodRes);

        // Prepare form and cart from fetched purchase data
        if (purchaseRes) {
          setForm({
            date: purchaseRes.date
              ? new Date(purchaseRes.date).toISOString().slice(0, 10)
              : todayStr(),
            supplier_id: purchaseRes.supplier_id || "",
            purchase_no: purchaseRes.purchase_no || "",
          });
          setCart(
            (purchaseRes.purchase_details || []).map((item) => ({
              id: item.id,
              product:
                prodRes.data?.find((p) => p.id === item.product_id) ||
                item.product ||
                {},
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await PurchaseService.update(id, payload);
      console.log(res);
      if (res && (res.success || res.status === "ok")) {
        ...
        setErrors({ general: "Failed to update purchase" });
      }
    } catch (err) {
      setErrors(
        err.response?.data?.errors || { general: "Failed to update purchase" }
      );
    }
    setSaving(false);
  };
