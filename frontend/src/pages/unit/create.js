...
const UnitCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const response = await UnitService.store(form);
      if (
        ...
        setErrors({ general: "Unit creation failed." });
      }
    } catch (err) {
      setErrors(
        err.response?.data?.errors || { general: "Failed to create unit." }
      );
    }
    setSaving(false);
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen py-8">
      <div className="max-w-3xl mx-auto px-6">
        <div className="mb-6">
          <h2 className="text-2xl font-semibold text-gray-700">Create Unit</h2>
