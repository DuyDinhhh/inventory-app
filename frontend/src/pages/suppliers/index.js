...
const Supplier = () => {
  ...
  // Fetch suppliers (all or search)
  const fetchSuppliers = async (_page = 1, searchTerm = "") => {
    setLoading(true);
    try {
      let response;
      if (searchTerm) {
        response = await SupplierService.search(searchTerm, _page);
      } else {
        response = await SupplierService.index(_page);
      }
  ...
  useEffect(() => {
    fetchSuppliers(page, isSearching ? searchInput : "");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.success("Supplier deleted successfully!", { autoClose: 800 });
      fetchSuppliers(page, isSearching ? searchInput : "");
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4 py-4">
      <div className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center my-6">
        <h2 className="text-2xl font-semibold text-gray-700">Suppliers</h2>
        <form
            ...
            {loading ? (
              ...
              suppliers.map((supplier) => (
                <tr key={supplier.id} className="text-gray-800">
                  ...
                  <td className="px-4 py-3">{supplier.shopname ?? "-"}</td>
                  <td className="px-4 py-3">{supplier.type ?? "-"}</td>
                  <td className="px-4 py-3">
                    <div className="flex flex-row items-center gap-2">
                      <Link
                        to={`/supplier/show/${supplier.id}`}
