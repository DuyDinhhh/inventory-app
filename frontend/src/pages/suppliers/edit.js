...
const SupplierEdit = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setImagePreview(data.photo || "/assets/img/suppliers/default.webp");
        setImageFile(null);
      } catch (err) {
        setSupplier(null);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await SupplierService.update(id, submitData);
      if (
        ...
        toast.success("Supplier updated successfully!", {
          autoClose: 700,
          onClose: () => navigate(`/supplier/show/${id}`),
        });
      } else {
        setErrors({
          general: "Supplier update failed.",
        });
      }
    } catch (err) {
      setErrors(
        err.response?.data?.errors || { general: "Failed to update supplier." }
      );
    }
    setSaving(false);
  };
